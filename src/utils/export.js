...
export function importFromJSON(event, setProject, setSelectedElement) {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log('Extracting images from imported JSON...');
        const extractedImages = cloudImageService.extractImagesFromJSON(migratedProject);
        ...
        .map(element => {
          if (element.type === 'text') {
            ...
            const textStyle = `
              ...
              font-family: '${element.properties.fontFamily || 'Arial'}', sans-serif;
              color: ${element.properties.color || '#000000'};
              ...
              ">
                <img src="${element.properties.src}" 
                     ...
                     style="
                       width: 100%;
                       height: 100%;
                       object-fit: ${element.properties.fit === 'stretch' ? 'fill' : (element.properties.fit || 'contain')};
                       display: block;
            ...
            switch (shapeType) {
              ...
              case 'arrow': {
                ...
                svgContent = `
                  <svg width="60%" height="100%" viewBox="0 0 ${w} ${h}" style="width: 60%; height: 100%; margin: 0 20%;">
                    ${shadow && shadowBlur > 0 ? `<defs><filter id="shape-shadow-${element.id}" x="-50%" y="-50%" width="200%" height="200%"><feDropShadow dx="0" dy="0" stdDeviation="${shadowBlur}" floodColor="${shadowColor}" floodOpacity="1" /></filter></defs>` : ''}
                    <line x1="${strokeOffset}" y1="${h/2}" x2="${arrowTip}" y2="${h/2}" stroke="${stroke}" stroke-width="${strokeWidth}" opacity="${opacity}" ${strokeDasharray !== 'none' ? `stroke-dasharray="${strokeDasharray}"` : ''} ${shadow && shadowBlur > 0 ? `filter="url(#shape-shadow-${element.id})"` : ''}/>
        ...
        <head>
          ...
          <style>
            ...
            * {
            ...
            div[style*="position: absolute"] > div:not([style*="width: 60%"]) {
              display: block !important;
...
export function exportToOptimizedSVG(project) {
  try {
    ...
    console.log('SVG Optimization Stats:', stats);
